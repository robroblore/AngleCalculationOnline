...
def max_values(vi, hi, dtheta, M, rA, ymin, dt, shape, D, G, vwind, terrain, wa, progress_bar, interface):
    """Get the max values of a given shot, varying the angle."""
    xmax_lst = []
    ymax_lst = []
    tmax_lst = []
    angl_lst = []
    for i in range(int(90/dtheta+1)):
        ...
        if progress_bar:
            progress_bar.set(i / int(90/dtheta))
...
def go(shape, terrain, D, rA, M, vi, theta, ymin, latitude, vwind, wa, dt, dtheta, progress_bar=None, interface=None):
    # shape = "blunt"  # Shape of the end of the projectile (blunt/conic)
    # terrain = "rough"  # Type of terrain below the projectile (impacts wind)
    # D = 0.1  # Diameter [m]
    # rA = 0.0134  # Reference area [m^2]
    # M = 30  # Mass [kg]
    # vi = 930  # Initial speed [m/s]
    # theta = 30  # Firing angle [°]
    # ymin = 5000  # Starting height [m]
    # latitude = 0  # Latitude on Earth [°]
    # vwind = 100  # Wind speed at ground level [m/s]
    # wa = 0  # Wind angle [°]
    # dt = 0.1  # Time interval [s]
    # dtheta = 1  # Angle interval [°]
    if latitude <= 30:
    ...
    xmax_lst, ymax_lst, tmax_lst, angl_lst = max_values(vi, ymin, dtheta, M, rA, ymin, dt, shape, D, G, vwind, terrain, wa, progress_bar, interface)
